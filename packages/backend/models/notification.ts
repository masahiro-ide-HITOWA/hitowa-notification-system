...
export type PushChannel = "google_chat" | "line" | "slack";
...
/**
 * 送信制御 Lambda が DynamoDB に保存・更新する通知アイテム。
 * テーブルキーは CDK 定義どおり employee_id / notification_id。
 */
export interface NotificationDeliveryItem extends NotificationPayload {
  employee_id: string;
  notification_id: string;
  status: NotificationDeliveryStatus;
  sent_at: string;
  expire_at: number;
  channel_results: ChannelSendResult[];
}
